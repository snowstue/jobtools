...
app.get("/api/storage/:key", (req, res) => {
  ...
  if (fs.existsSync(file)) {
    res.json({ key: req.params.key, value: fs.readFileSync(file, "utf8") });
  } else {
    res.json(null);
  }
});
...
app.post("/api/claude", async (req, res) => {
  if (!API_KEY) {
    return res.status(500).json({ error: "Set ANTHROPIC_API_KEY in Replit Secrets (lock icon in sidebar)" });
  }
  try {
...
const PORT = 3001;
app.listen(PORT, () => {
  console.log(`Colin AI API server running on :${PORT}`);
  console.log(`Anthropic key: ${API_KEY ? "✓ Set" : "✗ MISSING — add ANTHROPIC_API_KEY to Secrets"}`);
});
...
================================================================================
{
  "name": "colin-ai-job-intelligence",
  "private": true,
  "version": "1.0.0",
  "scripts": {
    "dev": "concurrently \"node api-server.js\" \"vite --host 0.0.0.0 --port 5000\"",
    "build": "vite build",
...
================================================================================
# Colin AI — Job Intelligence
## Replit Deployment (truly drop-in)
...
### Step 2: Upload files
Upload these 3 files to your Replit:

| File | Where it goes |
|------|---------------|
| `App.jsx` | Replace `src/App.jsx` (delete old contents, paste this) |
| `api-server.js` | Root of project (next to package.json) |
| `vite.config.js` | Replace existing `vite.config.js` |
...
### Step 4: Update the run command
In the Shell tab, open `.replit` file and change the `run` line to:
```
run = "npx concurrently \"node api-server.js\" \"npx vite --host\""
```

Or in package.json, change the "dev" script to:
```json
"dev": "concurrently \"node api-server.js\" \"vite --host\""
```
...
### Step 6: Hit Run
Everything works. Your data persists in the /data folder.

### Cost
~$0.01-0.05 per agent call. Typical daily use: $0.10-0.25.
